...
- **defconfig 模板**：`configs/*/br2_opentina.defconfig` — AArch64 + Bootlin 外部工具链 + **无内核**（内核由本仓库 `linux` 组件构建）+ **ext2 rootfs**。**OP-TEE 开启时**再编用户态 4.6.0（`BR2_PACKAGE_OPTEE_CLIENT` / `OPTEE_TEST` / `OPTEE_EXAMPLES`；`libteec`、`tee-supplicant`，SysV `S30tee-supplicant`；`xtest` / `optee_example_*` 的 TA 由 buildroot 自己的 TA devkit 编——devkit 只能由 **`BR2_TARGET_OPTEE_OS`** 产出，故该项开启但 **`_CORE=n`**，只出 devkit、不产与 `optee` 组件竞争的 `tee.bin`；`_SERVICES=n` 让 avb / pkcs11 等 in-tree service TA 不进 rootfs。optee_os 用 **`_CUSTOM_TARBALL`** pin 到 manifest 同一 commit，client / test / examples pin 到 4.6.0 tarball，hash 放在 `configs/common/br2-global-patches/`——不 pin 的话 buildroot 默认拉 4.9.0，与 4.6.0 的安全核错配）。BL32 仍由 **`optee`** 组件构建。**关闭 OP-TEE** 时配方会连 `BR2_TARGET_OPTEE_OS` 一起关掉。
- **TA**（仅 OP-TEE 开启）：`br2-post-build.sh`（以及 ubuntu/debian/yocto/openwrt 的镜像 overlay）把 `output/<BOARD>/optee` 里 **`export-ta_*/ta/*.ta`** 装到 **`/lib/optee_armtz`**；xtest / optee-examples 的 TA 由对应 BR 包自己产出并装进同一目录，不需要先编 **`optee`**。安全存储目录 **`/data/tee`**。改过 TA 后要重编对应 rootfs 并重烧 root 分区，只烧 `u-boot.fex` 不会更新它们。
